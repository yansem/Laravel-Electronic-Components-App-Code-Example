...
use App\Http\Resources\ComponentReferenceCollection;
...
class ComponentsReferenceResourceController extends Controller
{
    public function index(Request $request)
    {
        ...
        $items = ComponentReference::withFilters($request->all())
        ...
        return new ComponentReferenceCollection($items);
    }
    ...
    public function store(ComponentRequest $request)
    {
        ...
        $reference = ComponentReference::create($request->validated());

        History::create([
            'log_code_id' => LogCodes::COMPONENTS_ID,
            'user_id' => $this->userId,
            'operation_id' => Operation::ADD_ID,
            'historyable_type' => ComponentReference::class,
    ...
    public function update(ComponentRequest $request, $id)
    {
        ...
        $reference = ComponentReference::findOrFail($id);
    ...
    public function destroy($id)
    {
        ...
        $reference = ComponentReference::findOrFail($id);
        if ($reference->secureDelete('elements', 'groups', 'categories')) {
            History::create([
                'log_code_id' => LogCodes::COMPONENTS_ID,
                'user_id' => $this->userId,
    ...
    public function restore($id)
    {
        ...
        $reference = ComponentReference::withTrashed()->findOrFail($id);
        $reference->restore();
        History::create([
            'log_code_id' => LogCodes::COMPONENTS_ID,
            'user_id' => $this->userId,
    ...
    public function join(Request $request, $id)
    {
        ...
        $oldReference = ComponentReference::findOrFail($id);
        $newReference = ComponentReference::findOrFail($request->newId);

        $joinService = new JoinService($oldReference, $newReference, $this->userId);
        $joinService->join(
            [
                ['logCodeId' => LogCodes::ELEMENTS_ID, 'relation' => 'elements'],
                ['logCodeId' => LogCodes::GROUPS_ID, 'relation' => 'groups'],
                ['logCodeId' => LogCodes::CATEGORIES_ID, 'relation' => 'categories']
            ], LogCodes::COMPONENTS_ID);
